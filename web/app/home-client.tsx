...
export default function HomeClient({ initialCars }: { initialCars: DisplayCar[] }) {
  ...
  return (
    <main dir="rtl">
      ...
      ])}</tbody></table></div> : <div className="carGrid">{shownCars.map((car) => <article className="carCard" key={car.id}><a className={`carImage ${car.image ? "" : "noVehicleImage"}`} href={`/car/${car.id}`}>{car.image ? <img src={car.image} alt={`${car.make} ${car.model}`} loading="lazy" decoding="async" /> : <div><img src="/assets/logo1.png" alt="ענק הרכבים" loading="lazy" decoding="async" /><span>תמונה תעלה בקרוב</span></div>}<b>במלאי</b></a><div className="carInfo"><small>{car.category}</small><h3>{car.make} {car.model}</h3><p>שנת {car.year} | {car.gear || "אוטומטי"} | {Number(car.mileage || 0).toLocaleString("he-IL")} ק״מ</p><div><strong>{car.price.toLocaleString("he-IL")} ₪</strong><span>מחיר מבוקש</span></div>{car.monthly > 0 && <div className="cardMonthly"><strong>החל מ-{car.monthly.toLocaleString("he-IL")} ₪</strong><span>בחודש</span></div>}<a className="cardButton" href={`/car/${car.id}`}>לפרטים נוספים</a></div></article>)}</div>}
      ...
      <section id="sell" className="sellSection section"><div className="shell sellGrid"><div><span className="eyebrow">מוכרים רכב?</span><h2>אנחנו נקנה את הרכב שלכם</h2><p>השאירו פרטים וקבלו הצעה מהירה. אנחנו חוזרים רק לגבי רכבים שמעניינים אותנו לרכישה.</p><ul><li>תהליך מהיר ופשוט</li><li>העברת בעלות ותשלום במקום</li><li>ללא התחייבות מצדכם</li></ul></div><form onSubmit={sellLead.submit}><Honeypot /><div className="formGrid"><label>שם מלא<input required name="name" /></label><label>עיר<input required name="city" /></label><label>טלפון<input required name="phone" inputMode="tel" /></label><label>מספר רישוי<input required name="plate" inputMode="numeric" /></label><label>קילומטראז׳<input required name="mileage" inputMode="numeric" /></label><label>מחיר מבוקש<input required name="price" inputMode="numeric" /></label></div><button type="submit" disabled={sellLead.status === "sending"}>{sellLead.status === "sending" ? "שולח..." : "שליחת פרטים"}</button><LeadStatusMessage status={sellLead.status} error={sellLead.error} /></form></div></section>
